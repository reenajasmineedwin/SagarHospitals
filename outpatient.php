<?php
  ...
  //Diagnostic and Ancillary Services
   ...
   {  
    ...
    $query=mysqli_query($con,"insert into Personal_details(PatientName,Branch,Speciality,DoctorName,UHID_No,Gender,Age,MobileNo, LandlineNo,Emailid,Date) values('$PName','$branch','$speciality','$dr','$UHID','$Gender','$Age','$MobNo','$LandlineNo','$Email','$Date')");
                               ...
                               <div class="value">
                                <select class="input--style-6" style="margin-top: 2px; width:165px; height:38px;" name="branch" id="category-dropdown">
                                      <option value="">Select Branch</option>
                                        <?php
                                        $result = mysqli_query($con, "SELECT * FROM  Doctor_Data where parent_id = 0");
        ...
        <script>
            $(document).ready(function() {
                $('#category-dropdown').on('change', function() {
                    ...
                    $.ajax({
                        url: "outpatient_get-subcat_branch.php",
                        type: "POST",
                        data: {
        ...
        <script>
            $(document).ready(function() {
                $('#sub-category-dropdown1').on('change', function() {
                    var category_id = this.value;
                    $.ajax({
                        url: "outpatient_get-subcat_speciality.php",
                        type: "POST",
                        data: {
                            category_id: category_id
                        },
